    ...
    public function getstate($id,$type,$inv_id){
       ...
       // die($u->role_id.'    '. $id);
        ...
        if($this->startWith($id,'FNIF') )
        {
            ...
            if($u->role_id==2)
            $result=$this->Frenchies->find('all',['contain'=>['States']])->select(['States.id','States.name','Frenchies.name','Frenchies.address','Frenchies.gst'])->where(['Frenchies.id'=>$id]);
            else{
                $f=$this->Frenchies->get($u->username);
                $result=$this->Frenchies->find('all',['contain'=>['States']])->select(['States.id','States.name','Frenchies.name','Frenchies.address','Frenchies.gst'])->where(['Frenchies.id'=>$id,'Frenchies.frenchietype_id >'=>$f->frenchietype_id]);
        ...
        if($this->startWith($id,'FNI')  && $u->role_id==4 )
        {
            $this->loadModel('Frenchies');
            $f=$this->Frenchies->get($u->username);
